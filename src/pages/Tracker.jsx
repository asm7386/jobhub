...
import { saveJobs, loadLinks, saveLinks, loadPresets, savePresets } from "../utils/storage";
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave({ ...link, ...form }); }} className="space-y-3">
          ...
          <div className="flex items-center justify-between pt-1">
            <button type="button" onClick={() => onDelete(link.id)} className="px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg">Delete</button>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (!form.label.trim()) return; onSave({ ...form, id: Date.now().toString() }); }} className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Label *</label>
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" value={form.label} onChange={(e) => set("label", e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">URL</label>
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" value={form.url} onChange={(e) => set("url", e.target.value)} placeholder="https://..." />
            ...
            <div className="flex gap-2 flex-wrap">
              {LINK_COLORS.map((c) => (
                <button key={c} type="button"
                  className={`w-6 h-6 rounded-full ${c} ${form.color === c ? "ring-2 ring-offset-2 ring-indigo-500" : ""}`}
                  onClick={() => set("color", c)}
                />
              ))}
            </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm rounded-lg border border-gray-300 hover:bg-gray-50">Cancel</button>
            <button type="submit" className="px-4 py-2 text-sm rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">Add</button>
          </div>
        </form>
      </div>
    </div>
...
function QuickLinks() {
  ...
  function persist(next) { setLinks(next); saveLinks(next); }

  function handleSave(updated) { persist(links.map((l) => l.id === updated.id ? updated : l)); setEditing(null); }
  function handleDelete(id) { persist(links.filter((l) => l.id !== id)); setEditing(null); }
  function handleAdd(newLink) { persist([...links, newLink]); setAdding(false); }
    ...
    <>
      <div className="flex gap-3 mb-5 flex-wrap items-center">
        ...
        <button
          ...
          className="flex items-center justify-center w-10 h-10 rounded-xl border-2 border-dashed border-gray-300 text-gray-400 hover:border-indigo-400 hover:text-indigo-500"
          title="Add link"
        >
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-sm text-gray-500 mb-4">{incoming.length} row{incoming.length !== 1 ? "s" : ""} in file — {newItems.length} new, {dupes.length} duplicate{dupes.length !== 1 ? "s" : ""}.</p>
        {dupes.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-medium text-gray-500 mb-2">Duplicates (company + role already exists):</p>
        ...
        <div className="flex flex-col gap-2">
          {dupes.length > 0 && newItems.length > 0 && (
            <button onClick={() => onConfirm("skip")} className="w-full px-4 py-2 text-sm rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">
              Import {newItems.length} new (skip duplicates)
            </button>
          )}
          <button onClick={() => onConfirm("all")} className="w-full px-4 py-2 text-sm rounded-lg border border-gray-300 hover:bg-gray-50">
            Import all {incoming.length} (including duplicates)
          </button>
          <button onClick={onClose} className="w-full px-4 py-2 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-2 mb-4">
          <button disabled={rejected === 0} onClick={() => onCleanup("Rejected")}
            className="w-full flex justify-between items-center px-4 py-2.5 text-sm rounded-lg border border-gray-200 hover:bg-red-50 hover:border-red-200 disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          <button disabled={ghosted === 0} onClick={() => onCleanup("Ghosted")}
            className="w-full flex justify-between items-center px-4 py-2.5 text-sm rounded-lg border border-gray-200 hover:bg-gray-100 hover:border-gray-300 disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          <button disabled={rejected + ghosted === 0} onClick={() => onCleanup("both")}
            className="w-full flex justify-between items-center px-4 py-2.5 text-sm rounded-lg border border-gray-200 hover:bg-orange-50 hover:border-orange-200 disabled:opacity-40 disabled:cursor-not-allowed">
...
function JobModal({ job, onSave, onClose }) {
  ...
  function handleUrlChange(url) {
    setForm((f) => {
      ...
      return { ...f, jobUrl: url, source: !f.source && detected ? detected : f.source };
    });
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" value={form.source} onChange={(e) => set("source", e.target.value)} placeholder="LinkedIn, Indeed…" />
            </div>
            <div>
          ...
          <div>
            ...
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" value={form.jobUrl || ""} onChange={(e) => handleUrlChange(e.target.value)} placeholder="https://..." />
          </div>
          <div>
...
function JobRow({ job, onEdit, onDelete, jobs, setJobs, isSelected, onToggleSelect }) {
  ...
  function handleNotes(val) {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      const updated = jobs.map((j) => j.id === job.id ? { ...j, notes: val, updatedAt: now } : j);
      setJobs(updated);
      saveJobs(updated);
      setSavedLabel(true);
  ...
  function addLogEntry() {
    ...
    const updated = jobs.map((j) =>
      j.id === job.id ? { ...j, log: [...(j.log || []), entry], updatedAt: now } : j
    );
    setJobs(updated);
    saveJobs(updated);
    setLogText("");
          ...
          <td colSpan={10} className="px-6 py-4">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    className="flex-1 border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-300 bg-white"
                    placeholder="Add log entry…"
                    value={logText}
                    onChange={(e) => setLogText(e.target.value)}
                    onKeyDown={(e) => e.key === "Enter" && addLogEntry()}
                  />
                  <button onClick={addLogEntry} className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded border border-gray-200">+ Add</button>
                </div>
              </div>
            </div>
          </td>
...
export default function Tracker({ jobs, setJobs }) {
  ...
  function handleSave(form) {
    const now = new Date().toISOString();
    if (modal.mode === "add") {
      const newJob = { ...form, id: Date.now().toString(), updatedAt: now, log: [{ ts: now, text: "Application created" }] };
      ...
      const updated = jobs.map((j) => j.id === existing.id ? { ...existing, ...form, updatedAt: now, log } : j);
      setJobs(updated);
      saveJobs(updated);
    }
    setModal(null);
  ...
  function toggleSelectAll() {
    if (displayed.every((j) => selected.has(j.id))) {
      setSelected(new Set());
    } else {
  ...
  function handleBulkDelete() {
    ...
    const updated = jobs.filter((j) => !selected.has(j.id));
    setJobs(updated);
    saveJobs(updated);
    setSelected(new Set());
  }
  ...
  function handleBulkExport() {
    ...
    downloadCSV(jobsToCSV(selectedJobs), "jobhub_selected.csv");
  }

  // Cleanup
  function handleCleanup(mode) {
    let updated;
    if (mode === "Rejected") updated = jobs.filter((j) => j.status !== "Rejected");
    else if (mode === "Ghosted") updated = jobs.filter((j) => j.status !== "Ghosted");
    else updated = jobs.filter((j) => j.status !== "Rejected" && j.status !== "Ghosted");
    setJobs(updated);
    saveJobs(updated);
    setSelected(new Set());
  ...
  function importCSV(e) {
    ...
    reader.onload = (ev) => {
      ...
      const incoming = lines.filter(Boolean).map((line) => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        <div className="ml-auto flex gap-2 flex-wrap">
          ...
          <button onClick={() => setShowCleanup(true)} className="px-3 py-2 text-sm border border-red-200 text-red-600 rounded-lg hover:bg-red-50">Clean Up</button>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 mb-3 px-4 py-2.5 bg-indigo-50 border border-indigo-200 rounded-xl flex-wrap">
          ...
          <div className="flex items-center gap-1.5">
            <select value={bulkStatus} onChange={(e) => setBulkStatus(e.target.value)}
              className="border border-gray-300 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-400">
              <option value="">Change status…</option>
              {STATUSES.map((s) => <option key={s}>{s}</option>)}
            </select>
            {bulkStatus && (
              <button onClick={handleBulkStatusChange} className="text-xs px-2 py-1 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Apply</button>
            )}
          </div>
          <button onClick={handleBulkExport} className="text-sm text-gray-600 hover:underline">Export selected</button>
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {displayed.length === 0 ? (
              ...
              displayed.map((job) => (
                <JobRow
                  key={job.id} job={job} jobs={jobs} setJobs={setJobs}
